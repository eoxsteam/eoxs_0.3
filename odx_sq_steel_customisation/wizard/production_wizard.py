        ...
        for line in option_lines:
            ...
            if not line.is_job_order:
                ...
                if line.operation == 'slitting' or line.operation == 'cutting':
                    ...
                    #
                    if line.operation == 'slitting':
                        new_production.production_line_ids._onchange_width_in()
                    if line.operation == 'cutting':
                        new_production.production_line_ids._onchange_sheets()

                elif line.operation == 'multistage':
                    new_production.write({
                        'multi_stage_line_ids': [(0, 0, {
                            'production_stage_id': new_production.id,
                            'lot_id': line.lot_id.id,
                            'product_id': self.product_id.id,
                            'category_id': line.category_id.id,
                            'sub_category_id': line.sub_category_id.id,
                    ...
                    new_production.multi_stage_line_ids._onchange_width_in()
